<template>
  <div class="header-container">
    ...
    <div v-else class="btn-group">
      <v-btn class="login-btn mr-2 bg-primary" @click="login"> Login </v-btn>
      <v-btn @click="signUp"> Sign up </v-btn>
    </div>
    <LoginModal ref="loginModalRef" @sign-up="signUp" />
    <SignUpModal ref="singUpModalRef" @login="login" />
  </div>
  <vue-basic-alert ref="alert" :close-in="1500" />
</template>
<script setup>
...
const loginSuccess = (res) => {
  ...
  if (!res.isLoggedIn) {
    alert.value.showAlert("success", "Login Success.");
  }
  isLogged.value = true;
  loginModalRef.value.hide();
};
const loginFailed = () => {
  alert.value.showAlert("error", "Invalid Email or password", "Login Failed.");
...
const registerUserSuccess = () => {
  alert.value.showAlert("success", "Register Success.");
};
const registerUserFailed = () => {
  alert.value.showAlert("error", "Register Failed.");
};
</script>
